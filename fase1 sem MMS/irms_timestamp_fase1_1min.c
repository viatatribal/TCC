...
int main()
{
    ...
    uint64_t end_time = 60UL * 1000000000UL;
    int minutes_irms = 60*60;
    int minutes_timestamp = 80*60*60;
    ...
    FILE *arquivo;
    arquivo = fopen("irms1min.csv", "w");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo!\n");
        return 1;
    }

    fprintf(arquivo, "Irms1,");
    ...
    arquivo = fopen("timestamp1min.csv", "w");
    if (arquivo == NULL) {
        printf("Erro ao abrir o arquivo!\n");
        return 1;
    }

    fprintf(arquivo, "timestamp1,\n");
