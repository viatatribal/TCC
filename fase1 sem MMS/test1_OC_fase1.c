...
float Irms_pickup = 0;
float Irms_dropout = 0;
struct timespec pickup_event_time;
struct timespec dropout_time;
...
void OC_protection(float A, float B, float p, int i)
{
    if (isPickUpEvent[i] && Ip > Irms[i]) {
        clock_gettime(CLOCK_REALTIME, &dropout_time);
        Irms_dropout = Irms[i];
...
int main()
{
    ...
    uint64_t T3 = 25UL * 1000000000UL; // 25 seconds (10 from T1 + 8 from T2 + 7)
    ...
    printTime(dropout_time);
    printf(" %f A Proteção Sobrecorrente Temporizada Fase A Dropout \n", Irms_dropout);
